...
export default function FavoritesSlideOver() {
  ...
  if (!state.isFavoritesOpen) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      <div className="absolute inset-0 bg-black bg-opacity-50" onClick={() => dispatch({ type: 'CLOSE_FAVORITES' })} />
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-xl">
        <div className="flex h-full flex-col">
          ...
          <div className="flex items-center justify-between p-4 border-b border-gray-200">
            ...
            <button
              onClick={() => dispatch({ type: 'CLOSE_FAVORITES' })}
            ...
            {state.favorites.length === 0 ? (
              ...
              <div className="space-y-4">
                {state.favorites.map((favorite) => (
                  <div key={favorite._id} className="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg">
                    <img
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600">{formatPrice(favorite.product.price)}</p>
                    </div>
                    <button
